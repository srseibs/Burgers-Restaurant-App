 ...
 */
object ProfileValidationConstants {
 ...
 */
object ProfileFormValodator {
    fun validateFirstName(value: String): String? = RequiredLengthValidator(
        value = value,
        min = ProfileValidationConstants.NAME_MIN_LENGTH,
        max = ProfileValidationConstants.NAME_MAX_LENGTH
    ).validate()
    ...
    fun validateCity(value: String?): String? = OptionalLengthValidator(
        value = value,
        min = ProfileValidationConstants.CITY_MIN_LENGTH,
        max = ProfileValidationConstants.CITY_MAX_LENGTH
    ).validate()

    fun validateAddress(value: String?): String? = OptionalLengthValidator(
        value = value,
        min = ProfileValidationConstants.ADDRESS_MIN_LENGTH,
        max = ProfileValidationConstants.ADDRESS_MAX_LENGTH
    ).validate()
...
class PostalCodeValidator(private val value: Int?) : Validator {
    override fun validate(): String? {
        ...
        if (stringCode.length !in ProfileValidationConstants.POSTAL_CODE_MIN_LENGTH..ProfileValidationConstants.POSTAL_CODE_MAX_LENGTH) {
            return "Must be ${ProfileValidationConstants.POSTAL_CODE_MIN_LENGTH} to ${ProfileValidationConstants.POSTAL_CODE_MAX_LENGTH} characters"
        }
        return null
    }
}
...
class PhoneNumberValidator(private val value: PhoneNumber?) : Validator {
    override fun validate(): String? {
        if (value != null) {
            ...
            if (phoneString.length !in ProfileValidationConstants.PHONE_MIN_LENGTH..ProfileValidationConstants.PHONE_MAX_LENGTH) {
                return "Must be ${ProfileValidationConstants.PHONE_MIN_LENGTH} to ${ProfileValidationConstants.PHONE_MAX_LENGTH} characters"
            }
        }
        return null
    }
